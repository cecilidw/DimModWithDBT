...
from {{ ref('stg_pg__stores')}} as so
    left join {{ ref('stg_pg__staff')}} as sa on so.manager_staff_id = sa.staff_id 
        left join {{ ref('stg_pg__addresses')}} as a on so.address_id = a.address_id
